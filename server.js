...
app.post(
  ...
  (req, res) => {
    ...
    const images = (req.files["imageFiles"] || []).reduce((acc, file) => {
      ...
      acc[file.originalname] = `data:${mime};base64,${file.buffer.toString(
        "base64"
      )}`;
      return acc;
    }, {});
    ...
    fetch(url, {
      ...
      .then(async (pdfRes) => {
        console.log(
          `[PDF API] Response status: ${pdfRes.status} ${pdfRes.statusText}`
        );
        if (!pdfRes.ok) {
          let errText = "";
          try {
            errText = await pdfRes.text();
          } catch {}
...
async function checkPdfApiAndStart() {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      console.log(
        `[Startup] Checking PDF API health (attempt ${attempt}/${maxAttempts})...`
      );
      ...
      if (resp.ok || resp.status === 404) {
        ...
        console.error(
          `[Startup] PDF API not healthy: ${resp.status} ${resp.statusText}`
        );
      }
    } catch (err) {
